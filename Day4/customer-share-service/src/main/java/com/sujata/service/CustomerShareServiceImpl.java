...
public class CustomerShareServiceImpl implements CustomerShareService {
	...
	public List<CustomerShareDetail> getCustomerShareDetailByCustomerId(String id) {
		...
		Customers customers= restTemplate.getForObject("http://localhost:8084/customers/v1/"+id, Customers.class);
		for(Customer customer:customers.getCustomers()) {
			Share share=restTemplate.getForObject("http://localhost:8082/shares/v1/"+customer.getShareId(), Share.class);
